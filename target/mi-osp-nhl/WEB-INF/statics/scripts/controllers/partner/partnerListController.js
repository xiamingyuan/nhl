...
define(['../module'], function (controllers) {
    'use strict';
    controllers.controller('partnerListController', ['$scope', '$rootScope','$timeout','partnerService',function ($scope, $rootScope,$timeout,partnerService) {
        ...
        $scope.grid = {
            ...
            columnDefs: [
                {name: 'name', displayName: '名称',minWidth:80, headerCellClass: 'cell-center',
                    cellTemplate: '<div class=\'ui-grid-cell-contents\'><a ng-if="grid.appScope.permission.partnerDetail" href="#partner/detail/:{{row.entity.id}}"><span ng-bind="row.entity.name"></span></a><span ng-if="!grid.appScope.permission.partnerDetail" ng-bind="row.entity.name"></span></div>'
                },
                {name: 'id', displayName: '合作方id', minWidth: 80, headerCellClass: 'cell-center'},

                {name: 'secretKey', displayName: '秘钥',minWidth:170, headerCellClass: 'cell-center'},
                // {name: 'tel', displayName: '联系方式',minWidth:100, headerCellClass: 'cell-center', cellClass: 'cell-center'},
                ...
                {name: '_oper', displayName: '操作', width: 170, enableSorting: false, enableColumnResizing: false, headerCellClass: 'cell-center', cellClass: 'cell-center',
                    ...
                    '   <a ng-if="grid.appScope.permission.partnerDetail" href="#partner/detail/:{{row.entity.id}}">' +
                    '       <i class="icon-file-text-o"></i> 详情' +
                    '   </a>' +
                    '   <a ng-if="grid.appScope.permission.partnerEdit" href="#partner/edit/:{{row.entity.id}}" class="operate-a">' +
                    '      <i class="icon-edit"></i> 编辑' +
                    '   </a>' +
                    '   <a ng-if="grid.appScope.permission.partnerDelete" ng-click="grid.appScope.delhospital(row.entity.id)">' +
        ...
        $scope.gridOptions.getData = function(){
            ...
            partnerService.QueryHospitalList($scope.gridOptions.queryPrarms).success(function (data) {
                if (data.code == 200) {
        ...
        $scope.delhospital = function(id){
            bootbox.confirm("确定删除该合作方？", function (result) {
                if (result) {
                    partnerService.DelHospital({id:id}).success(function(data){
                        if(data.code==200){
        ...
        partnerService.getarea({ parentId: "0"}).success(function (data) {
            ...
            if($scope.searchParams){
                partnerService.getarea({ parentId: $scope.searchParams.province }).success(function (data) {
                    $scope.cityDatas = data.data;
                });
                partnerService.getarea({ parentId: $scope.searchParams.city }).success(function (data) {
        ...
        $scope.searchDepartment = function(id){
            partnerService.QueryDoctorgroupDoctorList({id:id}).success(function (data) {
        ...
        //获取专业科室列表
        partnerService.QueryMedicalDepartTree().success(function(data){
            if(data.code==200){
        ...
        $scope.delDepart = function(id){
            bootbox.confirm("确定删除该医生？", function (result) {
                if (result) {
                    partnerService.DelDoctorgroupDoctor({id:id}).success(function(data){
                        if(data.code==200){
                            $scope.searchDepartment($scope.hospitalDetail.id);
                        }else{
                            bootbox.alert(data.msg);
        ...
        $scope.doctorOptions.getData = function () {
            ...
            partnerService.QueryDoctorSelList($scope.doctorOptions.queryPrarms).success(function (data) {
                //返回的要渲染的数据
        ...
        $scope.saveDoctor =function () {
            //$scope.idList
            partnerService.InsertDoctorgroupDoctor({
        ...
        $scope.selectAll = function () {
            ...
            if($scope.isAll){
                $($scope.gridDoctor.data).each(function(){
                    this.selected = true;
        ...
        $scope.recordClick = function (e,entity) {
            ...
            if($scope.idList.length==$scope.gridDoctor.data.length){
                $scope.isAll=true;
            }else{
                $scope.isAll=false;
            }
