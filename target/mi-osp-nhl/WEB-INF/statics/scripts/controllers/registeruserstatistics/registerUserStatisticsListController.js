...
define(['../module'], function (controllers) {
     ...
     *
     * */
    controllers.controller('registerUserStatisticsListController', ['$scope', '$rootScope','registeruserstatisticsService',function ($scope, $rootScope,registeruserstatisticsService) {
        ...
        $scope.gridOptions.getData = function(){
            ...
            registeruserstatisticsService.query($scope.gridOptions.queryPrarms,"get").success(function (data) {
                if(data.code==200){
        ...
        $scope.bindHistogram = function (titleData, jsdata) {
            ...
            //var categories = ['总来电数', '进入IVR数', '进入队列数', '人工接听数', '进入队列挂断数', '进入队列等待数'];//暂时去掉非人工接听数
            var categories = arrX;
            ...
            var chart = $('#registeUsersChart').highcharts({
                chart: {
                    type: 'column'
                },
                title: {
