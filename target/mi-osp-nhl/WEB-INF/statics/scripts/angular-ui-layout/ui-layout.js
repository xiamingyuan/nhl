        ...
        function optionValue(option) {
          if(typeof option == 'number') {
              return option;
                ...
                if (option.match(sizePattern)) {
                    return option;
                } else {
                    return $scope.$eval(option);
                }
          } else {
            return null;
          }
          ...
          if(ctrl.containers.length > 0 && $element.children().length > 0) {
            ...
            for(i=0; i < ctrl.containers.length; i++) {
              if(!LayoutContainer.isSplitbar(ctrl.containers[i])) {
                  ...
                  c = ctrl.containers[i];
                opts.sizes[i] = c.isCentral ? 'auto' : c.collapsed ? (optionValue(c.minSize) || '0px') : optionValue(child.attr('size')) || 'auto';
