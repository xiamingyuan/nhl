...
define(['../module'], function (controllers) {
     ...
     *
     * */
    controllers.controller('billManageListController', ['$scope', '$http', '$rootScope','$timeout','strUtils','druglistService','financeService',function ($scope, $http, $rootScope,$timeout,strUtils,druglistService,financeService) {
        $scope.idList=[];//记录选择的账单列表的ID
        ...
        $scope.grid = {
            ...
            columnDefs: [
                ...
                {name: 'status', displayName: '账单状态',minWidth:100, cellFilter: 'AccountBillStatus:row.entity.businessConfirmStatus:row.entity.providerConfirmStatus',headerCellClass: 'cell-center', cellClass: 'cell-center'},
                {name: 'createTime', displayName: '创建时间',minWidth:160, cellFilter: 'date:"yyyy-MM-dd HH:mm:ss"', headerCellClass: 'cell-center', cellClass: 'cell-center'},
                {name: 'accountingTime', displayName: '到账日期',minWidth:140, cellFilter: 'date:"yyyy-MM-dd"',headerCellClass: 'cell-center', cellClass: 'cell-center'},
                {name: '_oper', displayName: '操作', width:80,enableSorting: false, enableColumnResizing:false, headerCellClass:'cell-center', cellClass: 'cell-center',
                    cellTemplate:
                    '<a ng-show="row.entity.businessConfirmStatus==\'0\' && row.entity.status==\'0\'&&grid.appScope.permission.billmngDeal" ng-click="grid.appScope.billAudit($event,row.entity)">' +
            ...
            onRegisterApi: function (gridApi) {
                ...
                $scope.gridApi.selection.on.rowSelectionChanged($scope, function (row) {
                    ...
                    if (rowIsSelected) {
                        ...
                        $scope.billgridOptions.queryPrarms.pageIndex=1;
        ...
        $scope.gridOptions.getData = function(){
            ...
            financeService.GetFinanceBill($scope.gridOptions.queryPrarms).success(function (data, status) {
                if(data.code==200){
        ...
        //账单确认操作
        $scope.billAudit = function (e, item) {
            if (item == null || item.id == undefined || item.id == null) {
                bootbox.alert("数据有误");
                return;
            }
            financeService.BusinessAuditPass({ id: item.id }).success(function (data, status) {
                if(data.code==200){
                    bootbox.alert(data.msg);
        ...
        $scope.billgrid = {
            ...
            enableRowHeaderSelection: false,
            columnDefs: [
                {name: 'claimSubmitId', displayName: '受理号',minWidth:100, headerCellClass: 'cell-center'},
                {name: 'providerName', displayName: '供应商/药店', minWidth:100,headerCellClass: 'cell-center'},
                {name: 'goodsName', displayName: '商品名称', minWidth:100,headerCellClass: 'cell-center',},
                {name: 'specification', displayName: '规格',minWidth:100, headerCellClass: 'cell-center'},
        ...
        $scope.exportBill = function () {
            bootbox.confirm('确定导出？', function (result) {
                if (result) {
                    var ids = "";
