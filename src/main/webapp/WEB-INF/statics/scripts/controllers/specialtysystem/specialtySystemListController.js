...
define(['../module'], function (controllers) {
     ...
     *
     * */
    controllers.controller('specialtySystemListController', ['$scope', '$rootScope', '$timeout', 'specialtySystemService',
        function ($scope, $rootScope, $timeout, specialtySystemService) {
            ...
            $scope.addChild = function () {
                $('#addModal').modal('show');
                $scope.org = {};
                $scope.childSystemLevelList = [];
                specialtySystemService.levelCompany().success(function (data) {
                    for (var i = 0; i < data.data.length; i++) {
                        if (data.data[i].itemvalue > $scope.CurrentNode.level_) {
                            $scope.childSystemLevelList.push(data.data[i]);
                        }
                    }
                    if ($scope.childSystemLevelList.length == 0) {
                        //判断如果最后一级==0，添加之后每一个子节点都是最低等级
                        $scope.childSystemLevelList.push(data.data[data.data.length - 1]);
            ...
            $scope.query = function (datas) {
                specialtySystemService.levelCompany().success(function (data) {
                    //获取等级列表
                    $scope.systemLevelList = data.data;
                    //获取数据
                    specialtySystemService.getChildSystemList({id: ""}).success(function (data) {
                        if (data.data.length != 0) {
                            ...
                            if (!$scope.CurrentNode) {
                                ...
                                specialtySystemService.getMedicalSystemByID({id:data.data[0].id}).success(function (orgData) {
            ...
            $scope.getChildData = function (Id, node,datas) {
                specialtySystemService.getChildSystemList({id: Id}).success(function (data) {
                    for (var i = 0; i < data.data.length; i++) {
                        if ($scope.selectedNodeId == data.data[i].id) {
                            $scope.orgDatail = data.data[i];
                            $scope.CurrentNode = data.data[i];
                            specialtySystemService.getMedicalSystemByID({id:data.data[i].id}).success(function (dataById) {
            ...
            $scope.createShow = function (org) {
                ...
                specialtySystemService.getMedicalSystemByID({id:org.id}).success(function (data) {
            ...
            $scope.create = function (valid, parent_id) {
                ...
                if (valid) {
                    ...
                    specialtySystemService.addSystem($scope.org).success(function (data) {
                        if (data.code == 200) {
                            if (parent_id == "") {
            ...
            $scope.edit = function (valid, org) {
                ...
                if (valid) {
                    if($scope.orgEdit.code == $scope.orgEdit.parent_code){
                        ...
                        $scope.isChild = false;
                        specialtySystemService.getMedicalSystemByCode({code:org.parent_code}).success(function (codeData){
                            if(codeData.code == 201){
                                $scope.serverErrorInfo = "您迁移的节点不存在！";
                                return;
                            }
                            org.parent_id = codeData.data.id;
                            specialtySystemService.queryMedicalSystem({id:org.parent_id}).success(function (dChild){
                                ...
                                if(!$scope.isChild){
                                    specialtySystemService.editSystem(org).success(function (data) {
                                        if (data.code == 200) {
                                            if ($scope.oldParentCode != null && $scope.oldParentCode == $scope.orgEdit.parent_code) {
                                                ...
                                                specialtySystemService.queryMedicalSystem({id:org.parent_id}).success(function (d){
                        ...
                        if (result) {
                            ...
                            if (org.hasChild) {
                                bootbox.confirm("含有子集目录，确定删除？", function (resultChild) {
                                    if (resultChild) {
                                        specialtySystemService.delete({id: org.id}).success(function (data) {
                                            //删除成功后 刷新列表
                                            if ($scope.arrList[org.parent_id] != null) {
                                                $scope.arrList[org.parent_id].isPlus = true;
                                                $scope.checkTreeClick($scope.arrList[org.parent_id]);
                                                specialtySystemService.getChildSystemList({id: $scope.arrList[org.parent_id].id}).success(function (data) {
