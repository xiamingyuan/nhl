...
define(['../module'], function (controllers) {
     ...
     *
     * */
    controllers.controller('cityListController', ['$scope', '$rootScope', '$timeout', 'cityService',
        function ($scope, $rootScope, $timeout, cityService) {
            ...
            $scope.addChild = function () {
                $('#addModal').modal('show');
                $scope.org = {};
                $scope.chlidAreaLevelList = [];
                cityService.levelArea().success(function (data) {
                    for (var i = 0; i < data.data.length; i++) {
                        if (data.data[i].itemvalue > $scope.CurrentNode.level_) {
                            $scope.chlidAreaLevelList.push(data.data[i]);
                        }
                    }
                    if ($scope.chlidAreaLevelList.length == 0) {
                        //判断如果最后一级==0，添加之后每一个子节点都是最低等级
                        $scope.chlidAreaLevelList.push(data.data[data.data.length - 1]);
            ...
            $scope.query = function (datas) {
                cityService.levelArea().success(function (data) {
                    //获取等级列表
                    $scope.areaLevelList = data.data;
                    //获取数据
                    cityService.getChildAreaList({id: "0"}).success(function (data) {
                        if (data.data.length != 0) {
                            ...
                            $scope.data = data;
                            $scope.cityCount = $scope.data.data.length;
            ...
            $scope.getChildData = function (Id, node,datas) {
                cityService.getChildAreaList({id: Id}).success(function (data) {
                    for (var i = 0; i < data.data.length; i++) {
                        if ($scope.selectedNodeId == data.data[i].id) {
                            $scope.orgDatail = data.data[i];
            ...
            $scope.createShow = function (org) {
                ...
                //页面右侧详细信息 展示
                $scope.orgDatail = org;
                $scope.oldParentId = org.parent_id;
            };
            ...
            $scope.create = function (valid, parent_id) {
                ...
                if (valid) {
                    ...
                    cityService.addArea($scope.org).success(function (data) {
                        if (data.code == 200) {
                            if (parent_id == "") {
                                $scope.selectedNodeId = $scope.org.id;//获取ID
                                $scope.query();
                            }
                            else {
            ...
            $scope.edit = function (valid, org) {
                ...
                if (valid) {
                    if($scope.orgEdit.oldId == $scope.orgEdit.parent_id){
                        $scope.serverErrorInfo = "城市迁移编码，不允许是当前城市编码！";
                        return;
                    }else {
                        $scope.isChild = false;
                        cityService.queryAreaByID({id:org.parent_id}).success(function (dChild){
                            var childList = dChild.data;
                            for(var i=0; i<childList.length; i++){
                                if(childList[i].id == org.oldId){
                                    $scope.isChild = true;
                            ...
                            if(!$scope.isChild){
                                cityService.editArea(org).success(function (data) {
                                    if (data.code == 200) {
                                        if($scope.oldParentId != null && $scope.oldParentId == $scope.orgEdit.parent_id){
                                            $scope.orgDatail = org;
                                            $scope.selectedNodeId = org.id;
                                            $scope.CurrentNode.isChecked = true;
                                            ...
                                            cityService.queryAreaByID({id:org.parent_id}).success(function (d){
                        ...
                        if (result) {
                            ...
                            if (org.hasChild) {
                                bootbox.confirm("含有子集目录，确定删除？", function (resultChild) {
                                    if (resultChild) {
                                        cityService.delete({id: org.id}).success(function (data) {
                                            //删除成功后 刷新列表
                                            if ($scope.arrList[org.parent_id] != null) {
                                                $scope.arrList[org.parent_id].isPlus = true;
                                                $scope.checkTreeClick($scope.arrList[org.parent_id]);
                                                cityService.getChildAreaList({id: $scope.arrList[org.parent_id].id}).success(function (data) {
